// Все физические константы теперь берутся из серверного Constants.java
const CONST = window.CONSTANTS;

// Константы визуализации (не имеют отношения к физике)
const FLASK_HEIGHT_PX = 350;
const PX_PER_METER = 400;
const MAX_WATER_M = FLASK_HEIGHT_PX / PX_PER_METER;
const START_LEVEL_M = 0.3;
const OVERFLOW_LEVEL_M = 0.8;
...
function updateInfoBlock() {
    ...
    const V_ball = (4/3) * CONST.PI * Math.pow(sphereRadius, 3);
    const r_flask = 0.15;
    const A_flask = CONST.PI * r_flask * r_flask;
    const h_water0 = 0.3;
...
function calculate() {
    ...
    const volume = (4/3) * CONST.PI * Math.pow(sphereRadius, 3);
    const mass = sphereDensity * volume;
    const gravityForce = mass * CONST.G;
    const archimedesForce = liquidDensity * volume * CONST.G;
    const stokesForce = 6 * CONST.PI * viscosity * sphereRadius * CONST.VELOCITY;
...
function updateRadiusDependencyChart() {
    const sphereDensity = parseFloat(sphereDensityInput.value) || CONST.DEFAULT_BALL_DENSITY;
    const liquidDensity = parseFloat(liquidDensityInput.value) || CONST.DEFAULT_LIQUID_DENSITY;
    const viscosity = parseFloat(viscosityInput.value) || CONST.DEFAULT_VISCOSITY;
    ...
    radii.forEach(r => {
        const volume = (4/3) * CONST.PI * Math.pow(r, 3);
        const mass = sphereDensity * volume;
        gravityData.push(mass * CONST.G);
        archimedesData.push(liquidDensity * volume * CONST.G);
        stokesData.push(6 * CONST.PI * viscosity * r * CONST.VELOCITY);
    });
...
function solveH(V, R) {
    if (V <= 0) return 0;
    const V_total = (4/3) * CONST.PI * Math.pow(R, 3);
    if (V >= V_total) return 2 * R;
    let low = 0, high = 2 * R;
    for (let i = 0; i < 50; i++) {
        const mid = (low + high) / 2;
        const V_mid = (CONST.PI / 3) * mid * mid * (3 * R - mid);
        if (V_mid < V) low = mid;
        else high = mid;
    }
    return (low + high) / 2;
...
function startSimulation() {
    ...
    const sphereDensity = parseFloat(sphereDensityInput.value) || CONST.DEFAULT_BALL_DENSITY;
    const sphereRadius = parseFloat(sphereRadiusInput.value) || CONST.DEFAULT_RADIUS;
    const liquidDensity = parseFloat(liquidDensityInput.value) || CONST.DEFAULT_LIQUID_DENSITY;
...
function createScale() {
    ...
    values.forEach(value => {
        ...
        if (value === MAX_WATER_M) displayValue = value.toFixed(3);
