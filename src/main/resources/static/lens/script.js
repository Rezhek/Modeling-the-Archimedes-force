(function() {
    ...
    function init() {
        ...
        [ui.f, ui.d, ui.h].forEach(el => el && el.addEventListener('input', (e) => {
            const idPart = e.target.id.split('-')[0];
            const numId = 'num-' + idPart;
            const numEl = document.getElementById(numId);
            if (numEl) numEl.value = e.target.value;
            update();
        }));

        [ui.nf, ui.nd, ui.nh].forEach(el => el && el.addEventListener('input', (e) => {
            const idPart = e.target.id.replace('num-', '');
            const rangeId = idPart + '-range';
        ...
        ui.type && ui.type.addEventListener('change', () => {
            currentObjColor = palette[Math.floor(Math.random() * palette.length)];
            update();
        });

        ui.lensType && ui.lensType.addEventListener('change', update);
    ...
    async function update() {
        if (!ui.f || !ui.d || !ui.h || !ui.lensType) return;
        ...
        try {
            ...
            const response = await fetch('/lens/api/update', {
                method: 'POST',
                headers: headers,
            ...
            if (rf) rf.innerText = lensModel.IsValid
                ? Math.abs(f_img).toFixed(1) + ' см' : '∞';
            if (rH) rH.innerText = lensModel.IsValid
                ? H_img.toFixed(1) + ' см' : '-';
            if (rg) rg.innerText = lensModel.IsValid
                ? mag.toFixed(2) : '-';
            if (rt) rt.innerText = lensModel.ImageType || '-';
    ...
    async function addMeasurement() {
        ...
        try {
            ...
            const response = await fetch('/lens/api/measurement/add', {
                method: 'POST',
                headers: headers,
    ...
    async function resetMeasurements() {
        ...
        try {
            ...
            const response = await fetch('/lens/api/measurement/reset', { 
                method: 'POST',
                headers: headers
            });
            const errorStats = await response.json();
    ...
    async function updateMeasurementsTable() {
        try {
            const response = await fetch('/lens/api/measurements');
    ...
    function updateErrorDisplay(errorStats) {
        const ef = document.getElementById('err-total-f');
        const eh = document.getElementById('err-total-h');
        if (ef) ef.innerText = errorStats.totalErrorF.toFixed(2) + ' см';
        if (eh) eh.innerText = errorStats.totalErrorH.toFixed(2) + ' см';
    }
    ...
    window.resetMeasurements = resetMeasurements;
